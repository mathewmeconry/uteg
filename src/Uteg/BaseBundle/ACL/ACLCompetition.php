...
class ACLCompetition
{
    ...
    public function getPermissionsByComp()
    {
        ...
        foreach ($this->acl->getObjectAces() as $ace) {
            ...
            $users[$ace->getSecurityIdentity()->getUsername()]['email'] = $this->em->getRepository('UtegBaseBundle:User')->findOneBy(array("username" => $ace->getSecurityIdentity()->getUsername()))->getEmail();
    ...
    {
        $admin = $this->em->getRepository('UtegBaseBundle:User')->findOneBy(array('username' => 'admin'));
    ...
    {
        $this->comp = $this->em->find('UtegBaseBundle:Competition', $this->requestStack->getCurrentRequest()->getSession()->get('comp'));
    ...
    {
        $user = $this->em->getRepository('UtegBaseBundle:User')->findOneBy($searchArray);
