...
class DefaultController extends Controller
{
    ...
    {
        return $this->forward('UtegBaseBundle:Redirecting:checkAuthentication', array("target" => "/competitions"));
    }
    ...
    {
        return $this->render('UtegBaseBundle::parseFlashbag.html.twig');
    }

    public function autocompleteStartersAction()
    {
        $em = $this->getDoctrine()->getManager();
        $starters = $em->getRepository('UtegBaseBundle:Starter')->findAll();
    ...
    public function addMassiveAction($competition, $starters, $club = null)
    {
        ...
        foreach ($starters as $starterPost) {
            if ($starterPost['firstname'] !== '' && $starterPost['lastname'] !== '' && $starterPost['birthyear'] !== '' && $starterPost['sex'] !== '' && (isset($starterPost['club']) !== '' || !is_null($club)) && $starterPost['category'] !== '') {
                $starter = $em->getRepository('UtegBaseBundle:Starter')->findOneBy(array("firstname" => $starterPost['firstname'], "lastname" => $starterPost['lastname'], "birthyear" => $starterPost['birthyear'], "sex" => $starterPost['sex']));
                if (!$starter) {
                    $starter = $em->getRepository('UtegBaseBundle:Starter')->findOneBy(array("lastname" => $starterPost['firstname'], "firstname" => $starterPost['lastname'], "birthyear" => $starterPost['birthyear'], "sex" => $starterPost['sex']));
                    if (!$starter) {
                        $starter = new Starter();
                ...
                if (is_null($club)) {
                    $club = $em->getRepository('UtegBaseBundle:Club')->find((array_key_exists('club', $starterPost)) ? $starterPost['club'] : 0);
                    if ($club !== 0 && $club) {
                        $s2c->setClub($club);
                    } else {
                ...
                $category = $em->getRepository('UtegBaseBundle:Category')->find((array_key_exists('category', $starterPost)) ? $starterPost['category'] : 0);
