...
use Uteg\BaseBundle\Entity\Clubs2Invites;
use Uteg\BaseBundle\Form\Type\C2iType;
...
class InviteController extends DefaultController
{
    ...
    public function inviteAction()
    {
        ...
        return $this->render('UtegBaseBundle::invite.html.twig');
    }
    ...
    public function inviteFormAction(Request $request)
    {
        ...
        $comp = $em->find('UtegBaseBundle:Competition', $request->getSession()->get('comp'));


        if ($request->request->get('id') !== null) {
            $club = $em->find('UtegBaseBundle:Club', $request->request->get('id'));
            $c2i = $em->getRepository('Uteg\BaseBundle:Clubs2Invites')->findOneBy(array("club" => $club, "competition" => $comp));
            if(!$c2i) {
            ...
            $club = $em->getRepository('Uteg\BaseBundle:Club')->findOneByName($request->request->get('uteg_invite')['club']);
            $c2i = $em->getRepository('Uteg\BaseBundle:Clubs2Invites')->findOneBy(array("club" => $club, "competition" => $comp));
            if(!$c2i) {
                $c2i = new Clubs2Invites();
                $c2i->setClub($club);
        ...
        return $this->render('UtegBaseBundle::form/inviteNewForm.html.twig',
        ...
        if (is_array($clubs)) {
            try {
                foreach ($clubs as $club) {
                    $clubobj = $em->find('UtegBaseBundle:Club', $club['id']);
                    $c2i = $em->getRepository('Uteg\BaseBundle:Clubs2Invites')->findOneBy(array("club" => $clubobj, "competition" => $em->find('UtegBaseBundle:Competition', $request->getSession()->get('comp'))));
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $c2i = $em->getRepository('Uteg\BaseBundle:Clubs2Invites')->findOneBy(array("token" => $token));
        $today = date('Y-m-d');

        if ($c2i->getValid()->format('Y-m-d') >= $today) {
            $categories = $em->getRepository('Uteg\BaseBundle:Category')->findBy(array(), array('number' => 'asc'));
            $qb = $em->createQueryBuilder();
            $qb->select('s2c')
                ->distinct()
                ->from('UtegBaseBundle\Entity\Starters2Competitions', 's2c')
                ->join('UtegBaseBundle\Entity\Competition', 'co', \Doctrine\ORM\Query\Expr\Join::WITH, 's2c.competition = co.id')
                ->where('s2c.club = ?1')
                ->andWhere('co.id = (SELECT MAX(s2c2.competition) FROM UtegBaseBundle\Entity\Starters2Competitions as s2c2 WHERE s2c2.club = ?2 AND s2c2.competition < ?3)')
                ->setParameter(1, $c2i->getClubObj())
                ->setParameter(2, $c2i->getClubObj())
                ->setParameter(3, $c2i->getCompetition());

            return $this->render('UtegBaseBundle::inviteClubView.html.twig',
    ...
    public function inviteAddTokenAction(Request $request, $token)
    {
        ...
        if ($c2i->getValid()->format('Y-m-d') >= $today) {
            ...
            if (isset($starters[0])) {
                ...
                $return = $this->addMassiveAction($em->find('UtegBaseBundle:Competition', $request->getSession()->get('comp')), $request->request->get('data'), $c2i->getClubObj());
                ...
                if (isset($return['fails'])) {
                    ...
                    return $this->render('UtegBaseBundle::inviteClubSuccess.html.twig');
                }
            } else {
    ...
    public function inviteListAction(Request $request)
    {
        ...
        return $this->render('UtegBaseBundle::inviteList.html.twig', array(
            "comp" => $this->getDoctrine()->getManager()->find('UtegBaseBundle:Competition', $request->getSession()->get('comp'))
        ));
    }
    ...
    public function inviteListPostAction(Request $request)
    {
        ...
        return $this->render('UtegBaseBundle::inviteList.json.twig', array(
            'source' => $result
        ));
    }
    ...
    {
        $this->get('acl_competition')->isGrantedUrl('CLUBS_EDIT');
        $c2i = $this->getDoctrine()->getManager()->find('UtegBaseBundle:Clubs2Invites', $id);
        ...
        return $this->render('UtegBaseBundle::form/inviteEditForm.html.twig',
            array('form' => $form->createView(),
                'target' => 'inviteEdit'
            )
        );
    ...
    public function inviteRemovePostAction(Request $request, $id)
    {
        ...
        $em = $this->getDoctrine()->getEntityManager();
        $c2i = $em->find('UtegBaseBundle:Clubs2Invites', $id);
    ...
    private function sendMail($default, $comp, $c2i, $message = null)
    {
        if ($default == 'true') {
            $mailmessage = str_replace("#FIRSTNAME#", $c2i->getFirstname(), str_replace("#LASTNAME#", $c2i->getLastname(), str_replace('#VALID#', $c2i->getValid()->format('Y-m-d'), str_replace('#COMPETITION#', $comp->getName(), str_replace("#INVITELINK#", $this->get('router')->generate('inviteToken', array(
                'token' => $c2i->getToken()
            ), true), $this->get('translator')->trans('invite.mail.message', array(), 'UtegBase'))))));
        } else {
        ...
        $mail = \Swift_Message::newInstance()
            ->setSubject($this->get('translator')->trans('invite.mail.subject %compname%', array('%compname%' => $comp->getName()), 'UtegBase'))
